...
if ($Cert) {
    ...
    ##### BEGIN Create Variable Library Array Of Module Functions As Strings #####

    $PrivateFunctionFileItems = @(Get-ChildItem -Path "$env:BHModulePath\Private" -File)
    $PublicFunctionFileItems = @(Get-ChildItem -Path "$env:BHModulePath\Public" -File)
    
    [System.Collections.Generic.List[String]]$AllModuleFunctions = @()
    foreach ($FunctionName in $PrivateFunctionFileItems.BaseName) {
        $null = $AllModuleFunctions.Add("    `${Function:$FunctionName}.Ast.Extent.Text")
    }
    foreach ($FunctionName in $PublicFunctionFileItems.BaseName) {
        $null = $AllModuleFunctions.Add("    `${Function:$FunctionName}.Ast.Extent.Text")
    }
    $FunctionsForScriptBlockUseHereDoc = @"
[System.Collections.ArrayList]`$script:FunctionsForSBUse = @(
$($AllModuleFunctions -join "`n")
)
"@
    $VariableLibraryTemplateContent = Get-Content "$env:BHModulePath\Private\VariableLibraryTemplate.ps1"
    $UpdatedVariableLibraryContent = $VariableLibraryTemplateContent -replace "'Array of Functions As Strings'",$FunctionsForScriptBlockUseHereDoc
    #Write-Host "Writing FunctionsForSBUse to $env:BHModulePath\VariableLibrary.ps1"
    #Write-Host "UpdatedVariableLibraryContent is:"
    #$UpdatedVariableLibraryContent
    Set-Content -Path "$env:BHModulePath\VariableLibrary.ps1" -Value $UpdatedVariableLibraryContent

    ##### END Create Variable Library Array Of Module Functions As Strings #####


    ##### BEGIN Tasks Unique to this Module's Build #####

    # Add Unicode Symbol Conversion Hashtable to VariableLibrary.ps1
    $UnicodeSymbolHTHereDoc = @"
...
"@
    ...
    if ($UpdatedVariableLibraryContent) {
        $UpdatedVariableLibraryContent = $UpdatedVariableLibraryContent -replace "'Unicode Symbol Converstion HashTable'",$UnicodeSymbolHTHereDoc
    }
    else {
        $VariableLibraryTemplateContent = Get-Content "$env:BHModulePath\Private\VariableLibraryTemplate.ps1"
        $UpdatedVariableLibraryContent = $VariableLibraryTemplateContent -replace "'Unicode Symbol Converstion HashTable'",$UnicodeSymbolHTHereDoc
    }
    #Write-Host "Writing 'Unicode Symbol Converstion HashTable' to $env:BHModulePath\VariableLibrary.ps1"
    #Write-Host "UpdatedVariableLibraryContent is:"
    #$UpdatedVariableLibraryContent
    Set-Content -Path "$env:BHModulePath\VariableLibrary.ps1" -Value $UpdatedVariableLibraryContent

    # Add Lua Scripts to VariableLibrary.ps1
    $LuaScriptFileItems = @(Get-ChildItem -Path "$env:BHModulePath\LuaScripts" -File)
    $LuaScriptsContentPSObjects = foreach ($FileItem in $LuaScriptFileItems) {
        $LuaScriptName = $FileItem.BaseName
        $LuaScriptContent = Get-Content $FileItem.FullName
        $PSObjHeredoc = @"
    [pscustomobject]@{
        LuaScriptName       = '$LuaScriptName'
        LuaScriptContent    = @'
$($LuaScriptContent -join "`n")
'@
    }
"@
        $PSObjHeredoc
    }
    $LuaScriptPSObjHereDoc = @"
[System.Collections.ArrayList]`$script:LuaScriptPSObjects = @(    
$($LuaScriptsContentPSObjects -join "`n")
)
"@

    if ($UpdatedVariableLibraryContent) {
        $UpdatedVariableLibraryContent = $UpdatedVariableLibraryContent -replace "'Array of Lua Script PSObjects'",$LuaScriptPSObjHereDoc
    }
    else {
        $VariableLibraryTemplateContent = Get-Content "$env:BHModulePath\Private\VariableLibraryTemplate.ps1"
        $UpdatedVariableLibraryContent = $VariableLibraryTemplateContent -replace "'Array of Lua Script PSObjects'",$LuaScriptPSObjHereDoc
    }
    #Write-Host "Writing 'Array of Lua Script PSObjects' to $env:BHModulePath\VariableLibrary.ps1"
    #Write-Host "UpdatedVariableLibraryContent is:"
    #$UpdatedVariableLibraryContent
    Set-Content -Path "$env:BHModulePath\VariableLibrary.ps1" -Value $UpdatedVariableLibraryContent
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQU4IvuLo30zLVPqKdqlAmu/m6x
# hkKgggn9MIIEJjCCAw6gAwIBAgITawAAAB/Nnq77QGja+wAAAAAAHzANBgkqhkiG
...
# NwIBCzEOMAwGCisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFJ/m5ySMsA6L+ZRV
# e4QrOnPXfPciMA0GCSqGSIb3DQEBAQUABIIBAI8T1J9dC67vJ35Eqv1DgxR1kOwe
# uLM5/TZ96EREi41mGe9W3njXjRcCHFLEEd3Ebd3M9vFtQjrv5SbMukKf/GWAOAmF
# 7ZE8F5TcEm2r6eLHMQPARuHKj2hPtMYX83VDEgV+KX6YJSZE53s7h3rjAbj0+hHY
# jTZ5d7GnP63ROfEzEeh7BJOoP4Z398TWRiHJM8sFDBxKU5j8yCR7B8w2OE4Sb/xS
# va8ov660NapO/tzL5R0ztO0G1fbJ/4fSeuIkAUiwsILAtulAlnuOVqZFgbOhqakh
# QncklX21MOMcuXVEdk5lgQyug/FVIrKxiojcF1AFDplDc45rAVnNJKOC790=
